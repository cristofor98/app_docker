...
async def register_client(client_socket: websockets.WebSocketServerProtocol, room_id, username):
    ...
    for client in chat_rooms[room_id]:
        if client.get("socket") != client_socket:
            await client["socket"].send(json.dumps({"type": "message", "username": username, "message": f"{username} {client_socket.remote_address[0]}:{client_socket.remote_address[1]} joined Room {room_id}!"}))
        else:
...
async def unregister_client(client_socket: websockets.WebSocketServerProtocol):
    ...
    for room_id in chat_rooms:
        for client in chat_rooms[room_id]:
            if client.get("socket") == client_socket:
                ...
                print(f" {username} {client_socket.remote_address} disconnected from Room {room_id}!")
    print(chat_rooms)
    ...
    for client in chat_rooms[room_id_c]:
        await client.get("socket").send(json.dumps({"type": "message", "username": username, "message": f" {username} {client_socket.remote_address[0]}:{client_socket.remote_address[1]} disconnected from Room {room_id_c}!"}))
...
async def process_message(message: str, client_socket: websockets.WebSocketServerProtocol):
    ...
    if data.get("type") == "join":
        ...
        print(f" {data.get('username')} joined room {data.get('room_id')}")
...
async def handle_client_connection(client_socket: websockets.WebSocketServerProtocol):
    ...
    print(f"{client_socket.remote_address} connected!")
